 ...
 */
public class WebSocketStreamHandler implements WebSockets.SocketListener, Closeable, com.ladeit.biz.websocket.terminal.WebSockets.SocketListener {
	private static final Logger log = LoggerFactory.getLogger(WebSocketStreamHandler.class);

	private final Map<Integer, TerminalInputStream> input = new HashMap<>();
	private final Map<Integer, TerminalOutputStream> pipedOutput = new HashMap<>();
	...
	public WebSocketStreamHandler(String poolid) {
		this.poolid = poolid;
	}
	...
	public synchronized void close() {
		if (state != State.CLOSED) {
			...
			for (TerminalOutputStream out : pipedOutput.values()) {
				try {
					out.close();
				} catch (IOException ex) {
	...
	public synchronized InputStream getInputStream(int stream) {
		...
		if (!input.containsKey(stream)) {
			TerminalInputStream pipeIn = new TerminalInputStream();
			TerminalOutputStream pipeOut = new TerminalOutputStream(this.poolid);
