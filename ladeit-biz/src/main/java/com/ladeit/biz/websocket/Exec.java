...
public class Exec {
	...
	public Process exec(
			String namespace, String name, String[] command, String container, boolean stdin, boolean tty, String poolid)
			throws ApiException, IOException {
		String path = makePath(namespace, name, command, container, stdin, tty);
		ExecProcess exec = new ExecProcess(apiClient,poolid);
		WebSocketStreamHandler handler = exec.getHandler();
		WebSockets.stream(path, "GET", apiClient, handler);
	...
	protected static class ExecProcess extends Process {
		...
		public ExecProcess(final ApiClient apiClient,String poolid) throws IOException {
			this.streamHandler =
					new WebSocketStreamHandler(poolid) {
						@Override
						protected void handleMessage(int stream, InputStream inStream) throws IOException {
							if (stream == 3) {
								int exitCode = parseExitCode(apiClient, inStream);
