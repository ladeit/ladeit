...
import static com.ladeit.util.istio.IstioSpecRegistry.getCRDNameFor;
...
public class YAML {
	...
	public static <T> List<T> loadIstioResources(final String specFileAsString, Class<T> clazz) {
		...
		if (!wantSpec && !IstioResource.class.isAssignableFrom(clazz)) {
			...
			try {
				...
				if (!document.isEmpty()) {
					...
					if (resourceYaml.containsKey(KIND)) {
						...
						final IstioResource resource = (IstioResource) objectMapper.convertValue(resourceYaml, clazz);
						if (wantSpec) {
